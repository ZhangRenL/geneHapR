    ...
    if (add.map) {
        ...
        if(!missing(Chr) | !missing(gff)){
            ...
            for(p in seq_len(length(ps))){
                for(i in seq_len(length(gff@ranges))){
                    start <- gff@ranges[i]@start
                    wid <- gff@ranges[i]@width
                    end <- start + wid
                    ...
                    if(stringr::str_detect(tolower(gff$type[i]),"cds"))
                        v <- 1 else v <- 1/2
                    xsp <- c(x1-w*v,x1+w*v,x2+w*v,x2-w*v) - w * p
                    ysp <- c(x1+w*v,x1-w*v,x2-w*v,x2+w*v) + w * p
                    xs <- c(xs, xsp, NA)
                    ys <- c(ys, ysp, NA)
                }
                xsp <- c(seq.x[1]-0.001,seq.x[1]+0.001,seq.x[2]+0.001,seq.x[2]-0.001)
                ysp <- c(seq.x[1]+0.001,seq.x[1]-0.001,seq.x[2]-0.001,seq.x[2]+0.001)
                xs <- c(xs,xsp - w,NA)
                ys <- c(ys,ysp + w,NA)
            }
