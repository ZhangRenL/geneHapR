...
#' @details
#' It is suggested that use the result of `vcf2hap()` or `seqs2hap`
#' as input directly.
#' However user can import hap result from file with `importHapResult()`
#'
#' @usage
#' hap_result(hap,
#'            hapPrefix = "H",
...
#' @param hap object of `haptypes` class, generated by `vcf2hap()`
#' or `seqs2hap` or `import_hapResult()`
...
#'
#' If hapresults were generated from vcf, `INFO` is null.
#' If hapresults were generated from vcf, `INFO` were same as in source vcf.
...
#'  ... ...
#'
#' However, the `INFO` in hap results may missing annotations that we need.
#' In this case, we can custom INFOs in hapresults with `addINFO()` or
#' `replaceINFO()`.
...
plotHapTable <- function(hapResult,
                         hapPrefix = "H",
                         title = "",
                         geneName = geneName,
                         INFO_tag = INFO_tag,
                         ...
                         ALLELE.color = "grey90")
{
    ...
    hps <-
        hapResult[grepl(paste0(hapPrefix, "[0-9]{1,}"), hapResult[, 1]), ] %>%
        as.matrix()
    if (nrow(hps) <= 1)
