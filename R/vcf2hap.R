...
#' @details
#' The class of returned object actually is `data.frame` with few
#' added `attributes` generated during processing and the data.frame itself,
#' such as `options`, `AccRemoved`, `AccRemain`, `AccAll`, `hap2acc`, `freq`.
#' Though, only a summary of the `attributes` were displayed, the full
#' information could be showed with `attr(x, "options")` or `attributes(x)`.
#' @usage
#' vcf2hap(vcf,
#'         hapPrefix = "H",
#'         filter_Chr = FALSE,
...
#' as `FALSE`. If set as `TRUE`, the `Chr` is needed.
...
vcf2hap <- function(
    vcf, hapPrefix = "H",
    filter_Chr = FALSE, Chr = Chr,
    filter_POS = FALSE, startPOS = startPOS, endPOS = endPOS,
    hyb_remove = TRUE,
    na.drop = TRUE,
    file = file) {
